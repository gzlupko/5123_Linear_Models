...
dat <- read_csv("/Users/gianzlupko/Desktop/5123 Linear Models/5123_Linear_Models/smokingRCT.csv") 
...
# with missing data.
dat3 <- subset(x = dat2, 
               select = c("ConditionF", "bdi_base", "bdi_EOT"))
dim(dat3)
print(dat3, n = 20)
...
(mns <- by(data = dat3$bdi_EOT,
           INDICES = dat3$ConditionF,
           FUN = mean))
(vars <- by(data = dat3$bdi_EOT,
            INDICES = dat3$ConditionF,
            FUN = var))
